...
class MysqlRepo:
    def __init__(self):
        ...
        self.password = os.getenv("MYSQL_PASSWORD", "0123")
        self.database = os.getenv("MYSQL_DB", "agriculture")
        ...
        try:
            with conn.cursor() as cur:
                sql = """
                           ...
                           LIMIT %s
                       """
                cur.execute(sql, (session_id, user_id, limit))
    ...
    def save_turn(
        ...
        sql = """
              INSERT INTO ai_chat_message (session_id, role, content, create_time)
              VALUES (%s, %s, %s, NOW())
          """
        try:
            with conn.cursor() as cur:
                cur.execute(sql, (session_id, "user", user_question))
                cur.execute(sql, (session_id, "assistant", assistant_answer))
        except Exception as e:
